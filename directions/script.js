var bud_P_M=` 
        <a href="drivers/ATP.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 5:15 </h2>
            <p class="AB">АВ Поставы</p>
            </div>
            <div class="stop2">
            <h2 class="time2">8:00</h2>
            <p class="AB">АВ Минск</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/KOBZIK.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 6:00 </h2>
            <p class="AB">Поставы</p>
            </div>
            <div class="stop2">
            <h2 class="time2">8:30</h2>
            <p class="AB">Минск</p>
            </div>
        </div>
        </a>
        <br>  
        <a href="drivers/CEREC.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 7:05 </h2>
            <p class="AB">АВ Поставы</p>
            </div>
            <div class="stop2">
            <h2 class="time2">9:45</h2>
            <p class="AB">АВ Минск</p>
            </div>
        </div>
        </a>
        <br>
         <a href="drivers/ATP.html"> 
         <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 9:00 </h2>
            <p class="AB">АВ Поставы</p>
            </div>
            <div class="stop2">
            <h2 class="time2">11:45</h2>
            <p class="AB">АВ Минск</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/VINTAKS.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 13:20 </h2>
            <p class="AB">АВ Поставы</p>
            </div>
            <div class="stop2">
            <h2 class="time2">17:20</h2>
            <p class="AB">АВ Минск</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/KOBZIK.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 13:45 </h2>
            <p class="AB">Поставы</p>
            </div>
            <div class="stop2">
            <h2 class="time2">16:10</h2>
            <p class="AB">Минск</p>
            </div>
        </div>
        </a>
        <br>
         <a href="drivers/ATP.html"> <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 16:10 </h2>
            <p class="AB">АВ Поставы</p>
            </div>
            <div class="stop2">
            <h2 class="time2">18:55</h2>
            <p class="AB">АВ Минск</p>
            </div>
        </div>
        </a>
        <br> `;
var pjt_P_M=` 
         <a href="drivers/ATP.html"> <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 5:15 </h2>
            <p class="AB">АВ Поставы</p>
            </div>
            <div class="stop2">
            <h2 class="time2">8:00</h2>
            <p class="AB">АВ Минск</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/KOBZIK.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 6:00 </h2>
            <p class="AB">Поставы</p>
            </div>
            <div class="stop2">
            <h2 class="time2">8:30</h2>
            <p class="AB">Минск</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/CEREC.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 7:05 </h2>
            <p class="AB">АВ Поставы</p>
            </div>
            <div class="stop2">
            <h2 class="time2">9:45</h2>
            <p class="AB">АВ Минск</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/ATP.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 9:00 </h2>
            <p class="AB">АВ Поставы</p>
            </div>
            <div class="stop2">
            <h2 class="time2">11:45</h2>
            <p class="AB">АВ Минск</p>
            </div>
        </div>
        </a>
        <br>
         <a href="drivers/ATP.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 12:00 </h2>
            <p class="AB">Поставы</p>
            </div>
            <div class="stop2">
            <h2 class="time2">13:30</h2>
            <p class="AB">Минск</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/VINTAKS.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 13:20 </h2>
            <p class="AB">АВ Поставы</p>
            </div>
            <div class="stop2">
            <h2 class="time2">17:20</h2>
            <p class="AB">АВ Минск</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/KOBZIK.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 13:45 </h2>
            <p class="AB">Поставы</p>
            </div>
            <div class="stop2">
            <h2 class="time2">16:10</h2>
            <p class="AB">Минск</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/CEREC.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 14:30 </h2>
            <p class="AB">АВ Поставы</p>
            </div>
            <div class="stop2">
            <h2 class="time2">17:05</h2>
            <p class="AB">АВ Минск</p>
            </div>
        </div>
        </a>
        <br>
         <a href="drivers/ATP.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 16:10 </h2>
            <p class="AB">АВ Поставы</p>
            </div>
            <div class="stop2">
            <h2 class="time2">18:55</h2>
            <p class="AB">АВ Минск</p>
            </div>
        </div>
        </a>
        <br> `;
var sub_P_M=` 
        <a href="drivers/ATP.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 5:15 </h2>
            <p class="AB">АВ Поставы</p>
            </div>
            <div class="stop2">
            <h2 class="time2">8:00</h2>
            <p class="AB">АВ Минск</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/CEREC.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 5:55 </h2>
            <p class="AB">АВ Поставы</p>
            </div>
            <div class="stop2">
            <h2 class="time2">8:30</h2>
            <p class="AB">АВ Минск</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/CEREC.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 7:05 </h2>
            <p class="AB">АВ Поставы</p>
            </div>
            <div class="stop2">
            <h2 class="time2">9:45</h2>
            <p class="AB">АВ Минск</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/KOBZIK.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 8:00 </h2>
            <p class="AB">Поставы</p>
            </div>
            <div class="stop2">
            <h2 class="time2">10:30</h2>
            <p class="AB">Минск</p>
            </div>
        </div>
        </a>
        <br>
         <a href="drivers/ATP.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 9:00 </h2>
            <p class="AB">АВ Поставы</p>
            </div>
            <div class="stop2">
            <h2 class="time2">11:45</h2>
            <p class="AB">АВ Минск</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/AP.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 13:20 </h2>
            <p class="AB">АВ Поставы</p>
            </div>
            <div class="stop2">
            <h2 class="time2">18:25</h2>
            <p class="AB">АВ Минск</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/KOBZIK.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 13:45 </h2>
            <p class="AB">Поставы</p>
            </div>
            <div class="stop2">
            <h2 class="time2">16:10</h2>
            <p class="AB">Минск</p>
            </div>
        </div>
        </a>
        <br>
         <a href="drivers/ATP.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 16:10 </h2>
            <p class="AB">АВ Поставы</p>
            </div>
            <div class="stop2">
            <h2 class="time2">18:55</h2>
            <p class="AB">АВ Минск</p>
            </div>
        </div>
        </a>
        <br> `;
var njd_P_M=` 
         <a href="drivers/ATP.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 5:15 </h2>
            <p class="AB">АВ Поставы</p>
            </div>
            <div class="stop2">
            <h2 class="time2">8:00</h2>
            <p class="AB">АВ Минск</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/CEREC.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 7:05 </h2>
            <p class="AB">АВ Поставы</p>
            </div>
            <div class="stop2">
            <h2 class="time2">9:45</h2>
            <p class="AB">АВ Минск</p>
            </div>
        </div>
        </a>
        <br>
         <a href="drivers/ATP.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 9:00 </h2>
            <p class="AB">АВ Поставы</p>
            </div>
            <div class="stop2">
            <h2 class="time2">11:45</h2>
            <p class="AB">АВ Минск</p>
            </div>
        </div>
        </a>
        <br>
         <a href="drivers/ATP.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 12:00 </h2>
            <p class="AB">АВ Поставы</p>
            </div>
            <div class="stop2">
            <h2 class="time2">14:45</h2>
            <p class="AB">АВ Минск</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/AP.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 13:20 </h2>
            <p class="AB">АВ Поставы</p>
            </div>
            <div class="stop2">
            <h2 class="time2">18:25</h2>
            <p class="AB">АВ Минск</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/KOBZIK.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 15:30 </h2>
            <p class="AB">Поставы</p>
            </div>
            <div class="stop2">
            <h2 class="time2">18:00</h2>
            <p class="AB">Минск</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/ATP.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 16:10 </h2>
            <p class="AB">АВ Поставы</p>
            </div>
            <div class="stop2">
            <h2 class="time2">18:55</h2>
            <p class="AB">АВ Минск</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/TIMYR.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 18:00 </h2>
            <p class="AB">Поставы</p>
            </div>
            <div class="stop2">
            <h2 class="time2">20:20</h2>
            <p class="AB">Минск</p>
            </div>
        </div>
        </a>
        <br>
        `;

var bud_M_P=`
        <a href="drivers/VINTAKS.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 7:40 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">11:40</h2>
            <p class="AB">АВ Поставы</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/ATP.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 9:47 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">12:45</h2>
            <p class="AB">АВ Поставы</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/KOBZIK.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 10:30 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">13:05</h2>
            <p class="AB">Поставы</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/KLJ.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 11:40 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">14:15</h2>
            <p class="AB">АВ Поставы</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/CEREC.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 12:15 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">14:50</h2>
            <p class="AB">АВ Поставы</p>
            </div>
        </div>
        </a>
        <br>
         <a href="drivers/ATP.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 13:17 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">16:15</h2>
            <p class="AB">АВ Поставы</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/KOBZIK.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 17:30 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">20:05</h2>
            <p class="AB">Поставы</p>
            </div>
        </div>
        </a>
        <br>
         <a href="drivers/ATP.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 20:37 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">23:35</h2>
            <p class="AB">АВ Поставы</p>
            </div>
        </div>
        </a>
        <br> `;
var pjt_M_P=`
        <a href="drivers/VINTAKS.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 7:40 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">11:40</h2>
            <p class="AB">АВ Поставы</p>
            </div>
        </div>
        </a>
        <br>
         <a href="drivers/ATP.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 9:47 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">12:45</h2>
            <p class="AB">АВ Поставы</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/KOBZIK.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 10:30 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">13:05</h2>
            <p class="AB">АВ Поставы</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/KLJ.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 11:40 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">14:15</h2>
            <p class="AB">АВ Поставы</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/CEREC.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 12:15 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">14:50</h2>
            <p class="AB">АВ Поставы</p>
            </div>
        </div>
        </a>
        <br>
         <a href="drivers/ATP.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 13:17 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">16:15</h2>
            <p class="AB">АВ Поставы</p>
            </div>
        </div>
        </a>
        <br>
         <a href="drivers/ATP.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 16:37 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">19:45</h2>
            <p class="AB">АВ Поставы</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/KOBZIK.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 17:30 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">20:05</h2>
            <p class="AB">АВ Поставы</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/TIMYR.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 18:00 </h2>
            <p class="AB">Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">20:20</h2>
            <p class="AB">Поставы</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/CEREC.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 18:45 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">21:20</h2>
            <p class="AB">АВ Поставы</p>
            </div>
        </div>
        </a>
        <br>
         <a href="drivers/ATP.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 20:37 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">23:35</h2>
            <p class="AB">АВ Поставы</p>
            </div>
        </div>
        </a>
        <br> `;
var sub_M_P=`
        <a href="drivers/AP.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 7:20 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">11:40</h2>
            <p class="AB">АВ Поставы</p>
            </div>
        </div>
        </a>
        <br>
         <a href="drivers/ATP.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 9:47 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">12:45</h2>
            <p class="AB">АВ Поставы</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/CEREC.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 10:30 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">13:05</h2>
            <p class="AB">АВ Поставы</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/KLJ.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 11:40 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">14:15</h2>
            <p class="AB">АВ Поставы</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/CEREC.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 12:15 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">14:50</h2>
            <p class="AB">АВ Поставы</p>
            </div>
        </div>
        </a>
        <br>
         <a href="drivers/ATP.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 13:17 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">16:15</h2>
            <p class="AB">АВ Поставы</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/KOBZIK.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 13:50 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">16:25</h2>
            <p class="AB">Поставы</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/KOBZIK.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 17:30 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">20:05</h2>
            <p class="AB">Поставы</p>
            </div>
        </div>
        </a>
        <br>
         <a href="drivers/ATP.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 20:37 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">23:35</h2>
            <p class="AB">АВ Поставы</p>
            </div>
        </div>
        </a>
        <br> `;
var njd_M_P=`
        <a href="drivers/AP.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 7:20 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">11:40</h2>
            <p class="AB">АВ Поставы</p>
            </div>
        </div>
        </a>
        <br>
         <a href="drivers/ATP.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 9:47 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">12:45</h2>
            <p class="AB">АВ Поставы</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/KLJ.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 11:40 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">14:15</h2>
            <p class="AB">АВ Поставы</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/CEREC.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 12:15 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">14:50</h2>
            <p class="AB">АВ Поставы</p>
            </div>
        </div>
        </a>
        <br>
         <a href="drivers/ATP.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 13:17 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">16:15</h2>
            <p class="AB">АВ Поставы</p>
            </div>
        </div>
        </a>
        <br>
         <a href="drivers/ATP.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 16:37 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">19:45</h2>
            <p class="AB">АВ Поставы</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/KLJ.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 17:50 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">20:25</h2>
            <p class="AB">АВ Поставы</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/KOBZIK.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 18:45 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">21:25</h2>
            <p class="AB">АВ Поставы</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/ATP.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 20:37 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">23:35</h2>
            <p class="AB">АВ Поставы</p>
            </div>
        </div>
        </a>
        <br>
        <a href="drivers/CEREC.html"> 
        <div class = "map" >
            <div class = "stop1" >
            <h2 class = "time1" > 22:00 </h2>
            <p class="AB">АВ Минск</p>
            </div>
            <div class="stop2">
            <h2 class="time2">00:35</h2>
            <p class="AB">АВ Поставы</p>
            </div>
        </div>
        </a>
        <br> `;

function f() {
    let now=new Date();
    let day = 1000 * 3600 * 24;
    let data;
    var select= document.getElementById("selectID");
    select.innerHTML+= '<option value="' + (now.getDay()) + '"> Сегодня</option>';
    for (var i=0; i<14;i++){
        now.setTime(now.getTime() + day);
        let days=now.getDate();
        let month=now.getMonth()+1;
        let weak='ВОСКРЕСЕНЬЕ,ПОНЕДЕЛЬНИК,ВТОРНИК,СРЕДА,ЧЕТВЕРГ,ПЯТНИЦА,СУББОТА'.split(',');
        if(days<10){
            days= "0"+days;
        }
        if(month<10){
            month="0"+month;
        }
        data=days + "." + month + " " + weak[now.getDay()];
        select.innerHTML+= '<option value="' + (now.getDay()) + '">' + data  + '</option>';
    }
    var list_P_M=document.getElementById('list_P_M');
    var list_M_P=document.getElementById('list_M_P');
    if (now.getDay()==5){
        if(list_P_M != null) {
            list_P_M.innerHTML = pjt_P_M;
        }
        if(list_M_P != null) {
            list_M_P.innerHTML = pjt_M_P;
        }
    }
    else if (now.getDay()==6){
        if(list_P_M != null) {
            list_P_M.innerHTML = sub_P_M;
        }
        if(list_M_P != null) {
            list_M_P.innerHTML = sub_M_P;
        }
    }
    else if (now.getDay()==0){
        if(list_P_M != null) {
            list_P_M.innerHTML = njd_P_M;
        }
        if(list_M_P != null) {
            list_M_P.innerHTML = njd_M_P;
        }
    }
    else{
        if(list_P_M != null) {
            list_P_M.innerHTML = bud_P_M;
        }
        if(list_M_P != null) {
            list_M_P.innerHTML = bud_M_P;
        }
    }
}

function cli() {
    var val= document.getElementById('selectID').value;
    var list_P_M=document.getElementById('list_P_M');
    var list_M_P=document.getElementById('list_M_P');
    if (val==5){
        if(list_P_M != null) {
            list_P_M.innerHTML = pjt_P_M;
        }
        if(list_M_P != null) {
            list_M_P.innerHTML = pjt_M_P;
        }
    }
    else if (val==6){
        if(list_P_M != null) {
            list_P_M.innerHTML = sub_P_M;
        }
        if(list_M_P != null) {
            list_M_P.innerHTML = sub_M_P;
        }
    }
    else if (val==0){
        if(list_P_M != null) {
            list_P_M.innerHTML = njd_P_M;
        }
        if(list_M_P != null) {
            list_M_P.innerHTML = njd_M_P;
        }
    }
    else {
        if(list_P_M != null) {
            list_P_M.innerHTML = bud_P_M;
        }
        if(list_M_P != null) {
            list_M_P.innerHTML = bud_M_P;
        }
    }
}